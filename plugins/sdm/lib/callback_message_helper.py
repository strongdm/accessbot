...
class CallbackMessageHelper:
    def __init__(self, bot, props):
        self.__bot = bot
        self.__admin_ids = bot.get_admin_ids(props.admins())
    ...
    def __is_valid_access_request(self, message):
        ...
        self.__bot.log.debug(f"************** sender_id: {message.frm} sender_message: {message.body}  admin _ids: {admin_ids} is_valid_access_request: {is_valid_access_request_grant}")
